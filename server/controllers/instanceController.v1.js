...
const Log = require('../models/Log'); // Implement
...
exports.put = async (req, res, next) => {
    ...
    if (mongoose.Types.ObjectId.isValid(instance._id)) {
        try {
            const data = await Instance.findByIdAndUpdate(instance._id, instance, { new: true, upsert: true, useFindAndModify: false });
            console.log('Instance Updated', instance._id);
            return res.send(data);
        } catch (error) {
            log.error = true;
            await log.save();
...
exports.reload = async (req, res, next) => {
    ...
    try {
        ...
        console.log('Box Reloaded', i._id);
        return res.send(i);
    } catch (e) {
        log.error = true;
        log.message = e;
